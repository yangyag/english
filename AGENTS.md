...
개인용 영단어 학습. 달력에 공부한 날과 신규·복습 개수를 기록한다. 하루 제한 없이 10개씩 학습하고, 마지막 신규 학습일의 단어를 선택적으로 복습한다. 본격 SRS는 넣지 않는다. 인증은 없다.
...
- 제출은 현재 묶음의 순위 전체다. 10장을 모아 한 번에 POST하며 남은 단어가 적으면 그 수만 제출한다.
...
- 복습하기와 새 단어 배우기를 자유롭게 선택한다. 미제출 응답은 브라우저에 임시 저장하고 완료 기록에는 포함하지 않는다.
