...
- 서버는 강제 `phase` 대신 신규·복습 목록과 복습 진행 상황을 제공한다. `TodayOut`의 `new_count`·`review_count`는 오늘(KST) 저장된 신규·복습 수다. 카드와 저장 안내의 오늘 누적 개수는 이 서버 집계를 사용하며 브라우저에서 별도로 증가시키지 않는다.
